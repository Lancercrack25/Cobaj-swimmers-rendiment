...
def interfaz_historial_terapias(root, nadador_id):
    ...
    def render_historial():
        ...
        for t in historial:
            ...
            # ← corregido: fecha_fin None = en curso
            en_curso = t['fecha_fin'] is None
            estado_color = "#F39C12" if en_curso else "#1ABC9C"
            estado_txt = "🕒 En curso" if en_curso else f"✅ Finalizada ({t['fecha_fin']})"
            ...
            if en_curso:
                def finalizar(tid=t['id'], tipo=t['tipo_terapia']):
                    ...
                    if ok:
                        talk(f"Terapia de {tipo} finalizada correctamente")
                        render_historial()
                    else:
                        print(f"Error: {msg}")
                ...
                ctk.CTkButton(row, text="Finalizar", width=100, height=32,
                              ...
                              command=lambda tid=t['id'], tipo=t['tipo_terapia']: finalizar(tid, tipo)

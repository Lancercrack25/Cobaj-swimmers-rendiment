...
def interfaz_eliminar_nadador(entrenador, root):
    ...
    def render(filtro=""):
        ...
        nadadores = obtener_nadadores(entrenador)
    ...
    def confirmar_eliminar():
        if not seleccionado["id"]:
            lbl_confirmacion.configure(
                text="⚠️ Selecciona un nadador primero",
                text_color="#F39C12"
            )
            return
        ...
        def confirmar():
            eliminar_nadador(seleccionado["id"], entrenador)
            dialogo.destroy()
            lbl_confirmacion.configure(
                text="✅ Nadador eliminado correctamente",
                text_color="#1ABC9C"
            )
            render(e_buscar.get().strip())

        ctk.CTkButton(
            btn_row, text="Sí, eliminar",
            fg_color="#E74C3C", hover_color="#C0392B",
            width=130, command=confirmar
        ).pack(side="left", padx=8)

        ctk.CTkButton(
            btn_row, text="Cancelar",
            fg_color="#2C3E50", hover_color="#1A252F",
            width=130, command=dialogo.destroy
        ).pack(side="left", padx=8)
